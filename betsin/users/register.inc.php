	...
	// Validate password strength
		...
		{
			$sql = "SELECT username FROM users WHERE username = ?"; //to be secured here, we are going to use prepared statement. so the user cant query in the website and destroy our database.
			...
			else
			{
				...
				*/
				...
				{
					$sql = "INSERT INTO users (username, email, pwdUser, firstname, lastname) VALUES (?, ?, ?, ?, ?)";
					$stmt = mysqli_stmt_init($conn);
					if (!mysqli_stmt_prepare($stmt, $sql)) {
						header("Location: register.php?error=sqlerror");
						exit();
